...
from loss import AdversarialLoss, FullLoss
...
class Trainer:
    ...
    def fit(self, train_hr_dir, eval_hr_dir, batch_size=32):
        ...
        for epoch in bar:
            ...
            print(f'Training losses: Generator: {gen_loss:.4f}, \
                  Discriminator: {disc_loss:.4f}')
            print(f'Generator score: ({gen_score:.3f}), \
                  Discriminator score: ({disc_score:.3f})')
            print('Singular losses:')
            img_l, adv_l, perc_l = self.gen_criterion.get_losses()
            print(
                f' Image loss = {img_l:.3f}; Adversarial loss = {adv_l:.3f}; \
                   Perception loss = {perc_l:.3f}')
    ...
    def _save_models(self, epoch=0):
        ...
        torch.save(self.disc_model.state_dict(), models_path / f'{d_name}.pt')
    ...
    def save_model(self, gen_model, gen_name, disc_model, disc_name):
        ...
        if not models_path.exists():
            models_path.mkdir()
        torch.save(gen_model.state_dict(), models_path / f'{gen_name}.pt')
        torch.save(disc_model.state_dict(), models_path / f'{disc_name}.pt')
...
if __name__ == "__main__":
    ...
    trainer = Trainer(crop_size=150, epochs=180, gen_optimizer=gen_optimizer,
